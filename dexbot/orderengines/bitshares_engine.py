...
class BitsharesOrderEngine(Storage, Events):
    ...
    def __init__(
        ...
        **kwargs
        ...
        if fee_asset_symbol:
            ...
            # If there is no fee asset, use PPY
            self.fee_asset = Asset('1.3.0', bitshares_instance=self.bitshares)
    ...
    def get_order_creation_fee(self, fee_asset):
        ...
        :param fee_asset: QUOTE, BASE, PPY, or any other
        :return:
        """
        # Get fee
        fees = self.dex.returnFees()
    ...
    def convert_fee(self, fee_amount, fee_asset):
        """
        Convert fee amount in PPY to fee in fee_asset.

        :param float | fee_amount: fee amount paid in PPY
        ...
        if fee_asset['id'] == '1.3.0':
            # Fee asset is PPY, so no further calculations are needed
            return fee_amount
        else:
